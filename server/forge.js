...
router.get('/forge/oauth/token', function (req, res) {
  var t = new token();
  t.getTokenPublic(function (tokenPublic, expires_in) {
    res.status(200).json({'access_token': tokenPublic, 'expires_in': expires_in});
  })
});

var ossBucketKey = process.env.FORGE_BUCKET || 'extractxlsbucket';
